...
bool OSCsender::init( String ip, int port ) {
	...
	if ( _ip.empty() || _port < 1 ) {
		std::cout << "OSCsender::init, invalid configuration: " << ipstring << ":" << _port << std::endl;
		return _ready;
	}
	_ready = true;
	...
	if ( !_started ) {
		...
		if ( !_ready ) {
			...
			try {
				...
				std::cout << "OSCsender::start, started successfully: " << ipstring << ":" << _port << std::endl;
				_started = true;
...
void OSCsender::set_autostart( bool autostart ) {
	_autostart = autostart;
}

void OSCsender::set_autoclear( bool autoclear ) {
...
void OSCsender::_bind_methods() {
	...
	ADD_PROPERTY(PropertyInfo(Variant::INT, "port", PROPERTY_HINT_RANGE, "1,99999,1"), "set_port", "get_port");
	ADD_PROPERTY(PropertyInfo(Variant::INT, "buffersize", PROPERTY_HINT_RANGE, "1,327680,1"), "set_buffersize", "get_buffersize");
